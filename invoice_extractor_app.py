...
def main_app():
    ...
    login_ui.show_user_info()
    login_ui.show_logout_button()
    
    st.title("📄 Invoice Extractor")
    st.markdown(f"Welcome, **{auth.get_current_user()}**! 👋")
            ...
            try:
                ...
                for i, d in enumerate(data):
                    d.update({
                        ...
                        "processed_by": auth.get_current_user(),
    ...
    if uploaded_files:
        ...
        **Processing Configuration**:
        ...
        if not pdf_data_list:
            ...
            if all_results:
                ...
                **Processing Summary**:
                ...
                • Processed by: {auth.get_current_user()}
                """)
                ...
                filename = f"invoice_extraction_{auth.get_current_user()}_{timestamp}.csv"
        ...
        login_str = time.strftime("%H:%M:%S", time.localtime(login_time)) if login_time else "N/A"
        
        st.markdown(f"""
        ### Welcome to Invoice Extractor, {auth.get_current_user()}!
        
        **Features**:
        - **Secure Access**: Protected by user authentication
        - **Multi-file Processing**: Process multiple PDF and image files simultaneously
        - **Parallel Processing**: Handle individual pages concurrently for faster extraction
        ...
        - **User Tracking**: All extractions are logged with user information
        - **Quality Analysis**: Confidence scoring for extracted data
        - **CSV Export**: Download results in structured format
        
        **Supported formats**: PDF, PNG, JPG, JPEG
        
        **Your Session**: Logged in as **{auth.get_current_user()}** at {login_str}
        """)
